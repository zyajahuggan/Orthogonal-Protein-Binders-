...
PROJECTS = [
    HbondProject("dhd", BENCHMARK_DIR / "metrics" / "dhd_metrics_filtered.csv", BENCHMARK_DIR / "outputs" / "dhd"),
    HbondProject("cop", BENCHMARK_DIR / "metrics" / "cop_metrics.csv", BENCHMARK_DIR / "outputs" / "cross_docking"),
]
...
with pymol2.PyMOL() as p:
    ...
    for project in PROJECTS:
        ...
        for job_id in metrics_df["project"]:
            cif_path = project.output_dir / job_id / "pred.model_idx_0.cif"
            if not cif_path.exists():
                print(f"Warning: no structure found for {job_id} ({project.name})")
                continue
            count = count_interface_hbonds(cmd, cif_path)
            hbond_counts[job_id] = count
            print(f"[{project.name}] {job_id}: {count} interface h-bonds")
