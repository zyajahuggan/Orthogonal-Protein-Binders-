"""Same computation as best_margin_comparison_with_rosetta.py (best metric per model, incl.
Rosetta's RIAM/FastRelax variants pooled across the 4 spms), but plotted as one figure with
dhd and cop side by side instead of two separate PNGs -- new file, does not modify or
overwrite best_margin_comparison_with_rosetta.py or its outputs.
"""
...
DATASET_ORDER = ["dhd", "cop"]  # left-to-right subplot order

# (metrics path, ID column, separator) per project per dataset -- copied from
# best_margin_comparison_with_rosetta.py
PATHS = {
    "AF3": {
...
def plot_panel(ax, dataset, best_metric_by_model, differentials_by_model):
...
for ax, dataset in zip(axes, DATASET_ORDER):
    ...
    plot_panel(ax, dataset, best_metric_by_model, differentials_by_model)
    all_summary_rows.extend(summary_rows)

axes[0].set_ylabel("Standardized differential (SD units)")
fig.suptitle("Best-margin metric per model, incl. Rosetta (pooled)")
plt.tight_layout()
plt.savefig(RESULTS_DIR / "best_margin_comparison_with_rosetta_combined.png", dpi=300, bbox_inches="tight")
plt.close()

summary_df = pd.DataFrame(all_summary_rows)
summary_df.to_csv(RESULTS_DIR / "best_margin_comparison_with_rosetta_combined.csv", index=False)
