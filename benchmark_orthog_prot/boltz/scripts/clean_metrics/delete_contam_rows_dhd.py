...
metrics_path = Path(__file__).resolve().parent.parent.parent / "metrics" / "boltz_metrics_dhd.csv"
output_path = metrics_path.with_stem(metrics_path.stem + "_filtered")
# -> boltz_metrics_dhd_filtered.csv, same folder

df = pd.read_csv(f"{metrics_path}")

patterns_to_drop = ['6a', '6b', '13_xaaaa', '13_xaaab']  # DHD designs found not truly orthogonal

mask = df['job_name'].str.contains('|'.join(patterns_to_drop), case=False, na=False)
df = df[~mask]
