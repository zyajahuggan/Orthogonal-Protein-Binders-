...
Runs once per project (cross_docking and dhd)."""
...
for df, csv_stem, project, sep, results_dir in iter_datasets():
    ...
    ax.boxplot([rank_df[m].values for m in metric_order], tick_labels=metric_order, showfliers=False)
    ...
    ax.set_title(f"{MODEL_NAME} Cognate pair ranking per metric — {project}")
