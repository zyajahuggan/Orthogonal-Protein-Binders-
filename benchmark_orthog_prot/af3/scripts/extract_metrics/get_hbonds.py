...
def count_interface_hbonds(cmd, cif_path):
    ...
    cmd.h_add("structure") #af3 doesnt have H's
...
class HbondProject(NamedTuple):
    name: str
    metrics_path: Path
    cif_for_sample: Callable[[str], Path]


# each sample's cif lives in a different place depending on the project:
# dhd was already collected into dhd_collected_cif_files/, cop is read
# straight out of its outputs/ tree
PROJECTS = [
    HbondProject(
        "dhd",
        BENCHMARK_DIR / "metrics" / "dhd_metrics_af3_filtered.csv",
        lambda sample: BENCHMARK_DIR / "dhd_collected_cif_files" / f"{sample}_model.cif",
    ),
    HbondProject(
        "cop",
        BENCHMARK_DIR / "metrics" / "cop_metrics_af3.csv",
        lambda sample: BENCHMARK_DIR / "outputs" / "cross_matching_orthogonal_prot" / sample / f"{sample}_model.cif",
    ),
]
