...
for df, csv_stem, label, sep, results_dir in iter_datasets():
    ...
    for metric in df.columns:
        ...
        std = scores.std()
        if std == 0:
            print(f"Warning: {metric!r} has zero variance in {label} -- skipping")
            continue
        ...
        for cog, non_cog in non_cognate_indexes.items():
            if not mask.loc[cog] or not all(mask.loc[indx] for indx in non_cog):
                continue  # metric missing data for this pair or one of its competitors
            margin = standardized.loc[cog] - max(standardized.loc[indx] for indx in non_cog)
            margins[df[ID_COL][cog]] = margin
        if margins:
            if len(margins) < len(non_cognate_indexes):
                print(f"{label}: {metric} margin computed for {len(margins)}/{len(non_cognate_indexes)} cognate pairs")
            full_margins[metric] = margins
        else:
            print(f"Warning: {metric!r} has no complete cognate/non-cognate pairs in {label} -- skipping")
    ...
    ax.set_title(f"Total score margin per metric, worst cognate vs best non-cognate — {label}")
