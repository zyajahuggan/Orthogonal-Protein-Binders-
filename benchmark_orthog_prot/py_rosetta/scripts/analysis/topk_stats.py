...
Runs once per project (cop and dhd)."""
...
for df, csv_stem, label, sep, results_dir in iter_datasets():
    ...
    for metric in df.columns:
        ...
        for cog, non_cog in non_cognate_indexes.items():
            if not mask.loc[cog] or not all(mask.loc[indx] for indx in non_cog):
                continue
            cog_score = oriented.loc[cog]
            rank = 1 + sum(oriented.loc[indx] > cog_score for indx in non_cog)
            ranking[df[ID_COL][cog]] = rank
    ...
    metric_order = [m for m in metric_order if rank_df[m].notna().any()]  # drop metrics with zero valid ranks
    rank_df = rank_df[metric_order]

    n_missing = rank_df.isna().sum()
    for m in metric_order:
        if n_missing[m] > 0:
            print(f"{label}: {m} missing rank for {n_missing[m]}/{len(rank_df)} cognate pairs")

    fig, ax = plt.subplots(figsize=(10, 6))
    ax.boxplot([rank_df[m].dropna().values for m in metric_order], tick_labels=metric_order, showfliers=False)

    rng = np.random.default_rng(0)  # fixed seed so the point jitter is reproducible
    for i, m in enumerate(metric_order, start=1):
        y = rank_df[m].dropna().values
    ...
    ax.set_title(f"{label}_Cognate pair ranking per metric ")
    plt.xticks(rotation=45, ha='right')
    ax.legend()
    plt.tight_layout()
    plt.savefig(ranking_dir / f"{label}_rank_boxplot.png", dpi=300)
    plt.close()
