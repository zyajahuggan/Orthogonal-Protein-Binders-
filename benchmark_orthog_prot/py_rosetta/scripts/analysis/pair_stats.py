...
recovers the same 14 per-design comparisons as the old cross_docking_pair_stats.py.

dhd: all 6 heterodimers are mutually orthogonal to each other, so every one of the
C(6, 2) = 15 combinations has matching cross rows.
...
for df, csv_stem, label, sep, results_dir in iter_datasets():
    ...
    for i, j in itertools.combinations(cognate_indices, 2):
        ...
        cross_rows = [
            ...
            if idx not in (i, j)
            and len(set(df.loc[idx, id_column].split(sep)) & chains_i) == 1
            and len(set(df.loc[idx, id_column].split(sep)) & chains_j) == 1
        ]
        if cross_rows:
            combos.append((i, j, cross_rows))

    print(f"--- {label}: {len(combos)} applicable combinations out of "
    ...
    for metric in df.columns:
        ...
        for i, j, cross_rows in combos:
            vals = [oriented.loc[i], oriented.loc[j]] + [oriented.loc[idx] for idx in cross_rows]
        ...
        valid_counts[metric] = len(successes)

    win_rate = pd.Series({
        metric: (sum(successes) / len(successes) if successes else float("nan"))
        for metric, successes in success_by_metric.items()
    }).sort_values(ascending=False)

    for metric, rate in win_rate.items():
        n_valid = valid_counts[metric]
        n_win = sum(success_by_metric[metric])
        print(f"{metric}: {n_win}/{n_valid} combinations ({rate:.0%}) had the worse "
    ...
    ax.set_ylim(0, 1)
    ax.set_title(f"Both-cognates-win rate per metric —({label})")
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig(barplot_dir / f"{label}_both_cognates_winrate.png", dpi=300, bbox_inches='tight')
    plt.close()