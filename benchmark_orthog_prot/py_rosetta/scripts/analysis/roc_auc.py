...
for df, csv_stem, label, sep, results_dir in iter_datasets():
    ...
    for metric in df.columns:
        ...
        auc = roc_auc_score(df.loc[mask, 'cognate_status'], scores[mask])
        results.append({'metric': metric, 'auc': auc})

    leaderboard = pd.DataFrame(results).sort_values('auc', ascending=False)
    leaderboard.to_csv(roc_auc_dir / f"{label}_auc_leaderboard.csv", index=False)
    ...
    for metric in leaderboard['metric']:
        scores = orient_scores(df, metric)
        mask = df[metric].notna()
        if mask.sum() < 2 or df.loc[mask, 'cognate_status'].nunique() < 2:
            continue  # not enough data or only one class present

        y_true = df.loc[mask, 'cognate_status']
        scores = scores[mask]
    ...
    plt.xlabel("AUC")
    plt.title(f"{label} AUC by Metric")
    plt.gca().invert_yaxis()
    plt.tight_layout()
    plt.savefig(roc_auc_dir / f"{label}_bar.png", dpi=300)
    plt.close()
