...
def main(args):
    ...
    # 1) Interface residues JSON
    if args.interface_residues_json:
        with open(args.interface_residues_json) as f:
            interface_residues= json.load(f)
    else:
        interface_residues = {}

    # 2) Chain map JSON
    if args.chain_map_json:
        with open(args.chain_map_json) as f:
            chain_letter_to_id = json.load(f)
    else:
        chain_letter_to_id = {}

    # 3) Tied pairs JSON
    if args.tied_pairs_json:
        with open(args.tied_pairs_json) as f:
            tied_pairs = json.load(f)
    else:
        tied_pairs = []
    ...
    with torch.no_grad():
        ...
        for ix, protein in enumerate(dataset_valid):
            ...
            # ----------------------------------------
            # Build PDB residue number lookup per chain
            # ----------------------------------------
            chain_ids = chain_encoding_all[0]  # [L]

            # === DEBUG: inspect chain C numbering ===
            cidC = chain_letter_to_id["C"]
            start = torch.where(chain_ids == cidC)[0].min()
            first_global = int(start)

            print("[DBG] first_global C =", first_global)
            print("[DBG] residue_idx C first 5 =",
                [int(residue_idx[0,i].item()) for i in range(first_global, first_global+5)])

            pdb_resnum_by_cid = {}
            for cid in torch.unique(chain_ids):
                cid = cid.item()
                idx = torch.where(chain_ids == cid)[0]

                # residue_idx may be 1000-based encoded → strip with %1000
                pdb_resnum_by_cid[cid] = [
                    (residue_idx[0, i].item() % 1000)
                    for i in idx
                ]
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--neg_weight",type=float,default=0.0,help="Negative weight applied to off-target chains at interface")
    argparser.add_argument("--pos_weight",type=float,default=0.8,help="Positive weight applied to target chains at interface")
    ...
    argparser.add_argument("--interface_residues_json", default="", help="Path to JSON interface residue file")
    argparser.add_argument("--chain_map_json", default="", help="Path to JSON chain map file")
    argparser.add_argument("--tied_pairs_json", default="", help="Path to JSON describing orthogonality relationships")
    argparser.add_argument("--num_repeats", type=int, default=1, help="Number of conditional sampling repeats")
    argparser.add_argument("--top_k",type=int,default=10,help="Number of top mutations to report per chain view")
