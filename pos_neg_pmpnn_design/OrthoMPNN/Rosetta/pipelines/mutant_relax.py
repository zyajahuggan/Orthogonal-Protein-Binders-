...
def parse_mutation_string(mut_string):
    """
    Convert a mutation string like:
        "R22K, N49L, R68L"
    into:
        [("A 22","K"), ("A 49","L"), ("A 68","L")]

    Rules:
      • residues 54 and 57 -> chain B
      • everything else -> chain A
    """
    muts = []
    parts = [p.strip() for p in mut_string.split(",")]
    ...
    for p in parts:
        ...
        muts.append((f"{chain} {pos}", mut))

    return muts

def load_mutation_bundles(file_path):
    import pandas as pd
    df = pd.read_csv(file_path)

    bundles = []

    for mut_string in df["Mutation Profile"]:
        # skip empty rows
        if pd.isna(mut_string):
            continue

        bundle = parse_mutation_string(mut_string)

        print(f"[DEBUG] Parsed {mut_string} -> {bundle}")
...
def job_list_creator(args):
    ...
    bundles = load_mutation_bundles(args.mutation_file)

    for bundle in bundles:
        print(f"[DEBUG] Creating jobs for bundle: {bundle}")
        mut_tag = "_".join([f"{pos.replace(' ','')}{aa}" for pos, aa in bundle])

        for i in range(1, args.nstruct+1):

            out_fname = f"{name_root}_{mut_tag}_rep{i}.pdb"
...
def parse_args():
    ...
    parser.add_argument("--out_dir", type=str, default="/scratch/jgray21/zyhuggan/Orthogonal-Protein-Binders-/pos_neg_pmpnn_design/OrthoMPNN/Rosetta/output_pdbs/r1/r1_enrich_wt", help="Output directory.")
    ...
    parser.add_argument("--mutation_file", type=str, default="/scratch/jgray21/zyhuggan/Orthogonal-Protein-Binders-/pos_neg_pmpnn_design/OrthoMPNN/pdgf/output/csvs/r1_enrich_ortho100seqs.csv", required=False, help="File containing mutation summary output")
